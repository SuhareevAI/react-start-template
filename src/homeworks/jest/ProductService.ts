...
const productCategories : ProductCategory[] = [
    { name: "Car", discount: [ {userType: "Gold", percent : 2}, {userType: "Premium", percent : 2} ]},
    { name: "Food", discount: [ {userType: "Premium", percent : 5} ]},
    { name: "Toy", discount: [ {userType: "Gold", percent : 10}, {userType: "Premium", percent : 10}, {userType: "Standard", percent : 10}]}
]
...
export class ProductService {
    ...
    getDiscount(userId : number, productCategory : productCategory) : number {
        var category = this.getCategory(productCategory);
        var user = this.accountService.getUser(userId);
        ...
        if (user && category && category.discount.length > 0) {
           var productDiscount = category.discount.filter((category) => category.userType == user.type.userType)?.[0];
